...
"""Uses watershed to sgment images with the addition of averaging out pixels in order to make a updates less sporatic."""
...
# img = cv2.resize(img, (img.shape[1]//5, img.shape[0]//5))
    ...
    _, thresh = cv2.threshold(img_gb, 180, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
...
while True:
    ...
    if len(f_list) > 4:
        for i in f_list:
             avg_frame = cv2.addWeighted(frame, 0.5, i, 0.5, 0)
        wframe = watershed(frame)
        cv2.imshow('win', wframe)
